...
class HeaderDataSource extends AbstractDataSource {
	...
	public function getStatusInformationHTML() {
		$html = __('Hosting Provider: ', 'geoip-detect') . get_option('geoip-detect-header-provider', 'None');
	...
	public function getParameterHTML() { 
		$provider = get_option('geoip-detect-header-provider');
		$checked_cloudflare = $provider == 'cloudflare';
		$checked_aws =        $provider == 'aws';

		$label = __('Hosting Provider:', 'geoip-detect');
		$html = <<<HTML
		<p>$label 
			<label><input type="radio" name="options_header[provider]" value="cloudflare" $checked_cloudflare /> Cloudflare</label>
			<label><input type="radio" name="options_header[provider]" value="cloudflare" $checked_aws /> Amazon AWS CloudFront</label>
	    </p>
		<br />	
HTML;
